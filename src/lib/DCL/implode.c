...
unsigned int infuncFile(char* buf, unsigned int* size, void* param) {
    unsigned int ret = fread(buf, 1, *size, ((InOut*) param)->infile);

    printf("infunc %u bytes | ret %u \n", *size, ret);

    return ret;
}

void outfuncFile(char* buf, unsigned int* size, void* param) {
    printf("outfunc %u bytes \n", *size);
    fwrite(buf, *size, 1, ((InOut*)param)->outfile);
}
...
unsigned int infuncBuff(char* buf, unsigned int* size, void* param) {
    rawBuffer* iBuffer = ((InOutBuffer*)param)->inBuffer;

    int tSize = iBuffer->length > *size ? *size : iBuffer->length;

    memcpy(buf, iBuffer->data, tSize);
...
void outfuncBuff(char* buf, unsigned int* size, void* param) {
    rawBuffer* oBuffer =  ((InOutBuffer*)param)->outBuffer;

    memcpy(&oBuffer->data[oBuffer->length], buf, *size);
    oBuffer->length += *size;
}
...
int compressBytes(char* input, int inLength,
    ...
    char* workBuf = calloc(1, CMP_BUFFER_SIZE);
    ...
    InOutBuffer ioBuffer;
    ioBuffer.inBuffer = &inBuffer;
    ioBuffer.outBuffer = &outBuffer;

    int err = implode(
        infuncBuff, outfuncBuff,
        workBuf, &ioBuffer,
        &compressType, &dictSize
    );
    if (err != 0)
        printf("ERROR CODE: %i \n\n", err);
...
int example_compress__() {
    char* workBuf = calloc(1, CMP_BUFFER_SIZE);
    unsigned int ctype = CMP_BINARY;
    unsigned int dictSize = CMP_IMPLODE_DICT_SIZE3;


    char inputData[] = "AIAIAIAIAIAIA";
    char* outputData = calloc(1, sizeof(inputData));

    rawBuffer inBuffer;
    inBuffer.data = inputData;
    inBuffer.length = sizeof(inputData);

    rawBuffer outBuffer;
    outBuffer.data = outputData;
    outBuffer.length = 0;

    InOutBuffer ioBuffer = { &inBuffer, &outBuffer };
    ...
    /*
    FILE* infile = fopen("zRawInput", "rb");
    FILE* outfile = fopen("zCompressed", "wb");

    InOut inOut = { infile, outfile };
    ...
    */
    ...
    FILE* f = fopen("zCompressed", "wb");
    fwrite(outBuffer.data, outBuffer.length, 1, f);
    fclose(f);
